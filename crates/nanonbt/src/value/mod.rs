...
///
/// Where fastnbt's conversion panics (`None`, units, newtype variants, and
/// malformed array wrappers), this returns an error.
...
impl ser::SerializeMap for SerializeCompound {
    ...
    fn serialize_value<T: Serialize + ?Sized>(&mut self, value: &T) -> Result<()> {
        ...
        self.compound.insert(key, to_value(value)?);
        Ok(())
    }
...
impl ser::SerializeStructVariant for SerializeStructVariant {
    ...
    fn serialize_field<T: Serialize + ?Sized>(
        ...
        self.compound.insert(String::from(key), to_value(value)?);
        Ok(())
    }
