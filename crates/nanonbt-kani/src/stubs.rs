...
//!   alignment, which Kani leaves symbolic. fastnbt runs `from_utf8` on
//!   every compound key and the `cesu8` crate `memchr` on every string;
//!   without them a struct of five fields exhausts 8 GB of memory, and a
//!   single array field does not finish in 3 minutes.
